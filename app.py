...
# --- CONFIG & FILENAMES ---
DATA_FILE = "ksd_master_v13.csv"
VE_FILE = "ksd_vehicles_v13.csv"
DR_FILE = "ksd_drivers_v13.csv"
    ...
    with st.expander("Add New Fuel Entry"):
        with st.form("fuel_f", clear_on_submit=True):
            ...
            if st.form_submit_button("Save Fuel"):
                ...
                st.success("Fuel Recorded!"); st.rerun()
...
# --- 4. STOCK IN ---
    ...
    with st.expander("Record Soil In"):
        with st.form("soil_f"):
            d = st.date_input("Date"); v = st.text_input("Supplier Vehicle"); qty = st.number_input("Cubes", min_value=0.0)
...
# --- 5. SALES OUT ---
    ...
    with st.expander("Record Sale Out"):
        with st.form("sale_f"):
            d = st.date_input("Date"); t = st.time_input("Time"); item = st.selectbox("Item", ["Sand Sale", "Soil Sale"])
            v = st.text_input("Transport Vehicle No"); qty = st.number_input("Cubes", min_value=0.0); amt = st.number_input("Amount", min_value=0.0)
            if st.form_submit_button("Record Sale"):
                new_r = pd.DataFrame([[len(df)+1, d, t.strftime("%H:%M"), "Income", item, v, "Sale", amt, qty, 0, 0, "Paid"]], columns=df.columns)
                df = pd.concat([df, new_r], ignore_index=True); df.to_csv(DATA_FILE, index=False)
                st.success("Sale Recorded!"); st.rerun()
