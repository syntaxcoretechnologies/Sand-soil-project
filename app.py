...
# --- CONFIG & DATABASE ---
DATA_FILE = "ksd_master_final_v5.csv"
...
if choice == "📊 Dashboard":
    ...
    with st.form("wash_f", clear_on_submit=True):
        d = st.date_input("Date")
        s_in = st.number_input("Soil Cubes Used", min_value=0.0)
        s_out = st.number_input("Sand Cubes Produced", min_value=0.0)
    ...
    with st.form("fleet_f", clear_on_submit=True):
        ...
        hrs = st.number_input("Hours Worked", min_value=0.0)
        cbs = st.number_input("Cubes Transported", min_value=0.0)
        amt = st.number_input("Cost (Rs.)", min_value=0.0)
...
# --- 6. EXPENSES & ADVANCES (FIXED SECTION) ---
    ...
    # Selection box outside the form to make it reactive
    ...
    with st.form("exp_f", clear_on_submit=True):
        ...
        # Cube box ONLY shows if "Soil Purchase" is selected above
        qty = 0.0
